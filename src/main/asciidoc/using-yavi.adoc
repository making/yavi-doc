...
TIP:: YAVI aggregates all violation messages by default. If a violation is found during the validation, it will not be shortcut. If you want to return from the current validation as soon as the first constraint violation occurs, use <<fail-fast-mode>>.
...
INFO:: This feature comes from https://docs.jboss.org/hibernate/stable/validator/reference/en-US/html_single/#section-fail-fast[the fail fast mode of Hibernate Validator.]
